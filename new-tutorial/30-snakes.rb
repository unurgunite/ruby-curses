...
class Sprites
  ...
  def get_sprite_value(offset)
    ...
    when 'X' then -4
    when '+' then 2
    when 'z' then -10 # dead (for the moment)
    else
      0
    end
  end
...
class PlayerXY
  ...
  def drawme
    ...
    setpos(tail[0], tail[1])
    addstr(SPACE)
    positions.each do |posxy|
      setpos(posxy[0], posxy[1])
      addstr(PCHAR)
    end
  end
...
class Paddle
  ...
  def paddle_draw
    ...
    if check_collision?
      if @howmany <= @hitctr
        setpos(REMARKSLINE, 36)
        addstr('YOU HAVE WON! Press q/Q to exit.')
        setpos(WONLOSTLINE, 36)
        addstr('YOU HAVE WON! Press q/Q to exit.')
        @gameover = true
      end
      if @player.lives <= 0
        setpos(REMARKSLINE, 36)
        addstr('YOU HAVE LOST ! Press q/Q to exit.')
        setpos(WONLOSTLINE, 36)
        addstr('YOU HAVE LOST ! Press q/Q to exit.')
        @gameover = true
      end
  ...
  def check_collision?
    ...
    sprites.each do |sp|
      ...
      if sp == head
        ...
        when -1
          setpos(REMARKSLINE, 0)
          addstr('O U C H! Eat apple to get well')
          setpos(STATUSLINE, 0)
          addstr('^ takes a life, @ adds life.  ')
        when 1
          setpos(REMARKSLINE, 0)
          addstr('B U R P! That was good!       ')
          setpos(STATUSLINE, 0)
          addstr('^ -1, @ +1, + +2, X -4        ')
        when 2
          setpos(REMARKSLINE, 0)
          addstr('Yummy ! That was great!       ')
        end
...
begin
  ...
  screen.box(0, 0)
  setpos(0, 30)
  addstr('Snakes with Ruby/Curses')
  ...
  loop do
    ...
    when Key::RIGHT then paddle.right
    when Key::LEFT then paddle.left
    end
    paddle.puttime
    paddle.paddle_draw
  end
