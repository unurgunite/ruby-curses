...
Border = Struct.new(:ls, :rs, :ts, :bs, :tl, :tr, :bl, :br) do
  def self.sql_style
    new('|', '|', '-', '-', '+', '+', '+', '+')
  end

  NAME_MAP = { 't' => 'top', 'r' => 'right', 'b' => 'bottom', 'l' => 'left', 's' => 'edge' }.freeze
...
Window = Struct.new(:startx, :starty, :height, :width, :border) do
  ...
  def border_coordinates
    yvals = (starty..endy).to_a
    xvals = (startx..endx).to_a
    yvals.product(xvals)
  end
