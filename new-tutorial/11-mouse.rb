...
CHOICES = ['Choice 1', 'Choice 2', 'Choice 3', 'Choice 4', 'Exit'].freeze
...
def print_menu(menu_win, highlight)
  ...
  CHOICES.each_with_index do |_choice, i|
    if highlight == i + 1
...
def report_choice(mouse_x, mouse_y)
  ...
  CHOICES.each_with_index do |choice, k|
    next unless mouse_y == j + k && mouse_x >= i && mouse_x <= i + choice.length
    return -1 if k == CHOICES.length - 1
...
begin
  ...
  loop do
    ...
    when Ncurses::KEY_MOUSE
      if Ncurses.getmouse(event) == Ncurses::OK && (event.bstate & Ncurses::BUTTON1_PRESSED)
