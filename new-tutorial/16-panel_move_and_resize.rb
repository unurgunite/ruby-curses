...
class CustomPanel
  ...
  def method_missing(method, *args, &block)
    panel.__send__(method, *args, &block)
  end
end
...
def win_show(win, label, label_color)
  ...
  Ncurses.box(win, 0, 0)
  Ncurses.mvwaddch(win, 2, 0, '+'.ord)
  Ncurses.mvwhline(win, 2, 1, '-'.ord, width - 2)
  Ncurses.mvwaddch(win, 2, width - 1, '+'.ord)
...
def print_in_middle(win, starty, startx, width, string, color)
  ...
  y = Ncurses.getcury(win)
  x = Ncurses.getcurx(win)

  x = startx != 0 ? startx : x
  y = starty != 0 ? starty : y

  width = width.zero? ? 80 : width
